...
Require Import Lia.  (* Linear Integer Arithmetic *)
...
(* Predecessor function with error *)
...
(* Coherencia de las conversiones con el sucesor  *)
...
- simpl.
  ...
  (*   rewrite <- plus_n_O.
  rewrite <- plus_Sn_m.
  replace (S (toN b + toN b)) with (toN b + toN b + 1).
  + change 2 with (1+1).
    rewrite plus_assoc.
    reflexivity.
(*   Search (S _ = _ + 1). *)
...
(* A ; B  aplica B a todas las metas generadas por A *)
...
Lemma sucBNplus_idem_U: forall (b : BN),  sucBN (b ⊞ b) = U b.
Proof.
intros.
induction b.
- simpl.
...
Lemma sucBNplus_sucBNbb_D: forall (b : BN),  sucBN (sucBN b ⊞ b) = D b.
Proof.
intros.
induction b.
- simpl.
...
(* intro.
rewrite plus_sucBNbb.
simpl.
reflexivity.
 *)  
(* Lemma plus_sucBNbb: forall (b:BN), (sucBN b) ⊞ b = U b.
Proof.
induction b.
- simpl. reflexivity.
- simpl. rewrite sucBNplus_idem_U. reflexivity.
- simpl. rewrite IHb. simpl. reflexivity.
Qed. *)



Lemma sucBNplus : forall (b c: BN), sucBN (b ⊞ c) = sucBN b ⊞ c.
Proof.
intros b.
induction b.
- simpl.
...
(* Coherencia de las conversiones con la suma *)
...
Lemma plus_Z: forall (b:BN), b ⊞ Z = b.
Proof.
intros.
destruct b.
- simpl;trivial.
